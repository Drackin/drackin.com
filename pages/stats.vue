        ...
        <section class="w-full max-h-full flex-wrap flex justify-center sm:items-center">
            ...
            <div v-if="lanyard" class="shadow transition m-2 hover:-translate-y-1 w-96 h-36 rounded-xl flex p-2 space-x-2 bg-white bg-opacity-10">
                <aside class="flex shrink-0 h-full items-center">
                    <img :src="`https://cdn.discordapp.com/app-assets/${lanyard.application_id}/${lanyard.assets!.small_image}.png`" alt="Album Art" class="w-32 h-32 rounded-lg object-cover" />
                </aside>
                <header class="h-full w-full text-sm flex flex-col">
                    ...
                    <p class="text-lg font-semibold">{{ lanyard.name }}</p>

                    <p class="text-sm whitespace-pre-line">{{ lanyard.details }}</p>

                    <p class="text-gray-300 text-sm whitespace-pre-line">{{ lanyard.state }}</p>
                </header>
            </div>
            ...
            >
                <aside class="flex shrink-0 h-full items-center">
                    <NuxtImg id="spoti-img"  :src="track?.coverArt" placeholder="/spotify.svg" alt="Album Art" class="w-32 h-32 rounded-lg object-cover" />
                </aside>

                <header class="h-full w-full text-sm flex flex-col">
                    <p class="flex items-center">
                        {{ spoti.isPlaying ? "Listening to Spotify" : "Recently Played" }}
                        <Icon name="simple-icons:spotify" class="mx-1" />
                    </p>

                    <p class="font-semibold text-lg line-clamp-2">{{ track?.name }}</p>

                    <p class="text-sm text-gray-400 whitespace-pre-line">by {{ track?.artist }}</p>

                </header>
            </a>


            <div class="shadow transition m-2 p-2 hover:-translate-y-1 flex flex-col justify-between items-center w-96 h-36 bg-white bg-opacity-10 rounded-xl">
            ...
            <div class="shadow transition m-2 p-2 hover:-translate-y-1 flex flex-col justify-between items-center w-96 h-36 bg-white bg-opacity-10 rounded-xl">
                ...
                <div class="w-full h-24 flex space-x-2">
                    <aside class="flex shrink-0 h-full items-center">
...
const track = spoti.isPlaying ? spoti.currentlyPlaying : spoti.recentlyPlayed
